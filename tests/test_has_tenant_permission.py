...
from tenant2fast_fastapi.models.route_model import TenantRoute
...
async def setup_rbac(tenant_session, test_user):
    ...
    t_user = TenantUser(auth_user_id=test_user.id, position="Manager")
    tenant_session.add(t_user)
    await tenant_session.commit()
    await tenant_session.refresh(t_user)

    # 2. Create Permission & Route
    route = TenantRoute(path="/test/permission", method="GET")
    tenant_session.add(route)
    await tenant_session.flush()
    
    perm = TenantPermission(name="TestPermission")
    tenant_session.add(perm)
    await tenant_session.flush()
    
    link_route = TenantPermissionRoute(permission_id=perm.id, route_id=route.id)
    tenant_session.add(link_route)
    
    # 3. Create Role and assign permission
    role = TenantRole(name="TestPermissionRole")
    tenant_session.add(role)
    await tenant_session.flush()
    
    link_role_perm = TenantRolePermission(role_id=role.id, permission_id=perm.id)
...
async def test_permission_granted_via_role(test_user, test_tenant, user_tenant_link, setup_rbac, tenant_session):
    ...
    # Assign role to user
    assignment = TenantUserRole(user_id=setup_rbac["user"].id, role_id=setup_rbac["role"].id)
...
async def test_permission_pattern_matching(test_user, test_tenant, user_tenant_link, tenant_session, setup_rbac):
    ...
    route = TenantRoute(path="/test/roles/{role_id}", method="DELETE")
    tenant_session.add(route)
    await tenant_session.flush()
    
    # 2. Link to existing permission
    link = TenantPermissionRoute(permission_id=setup_rbac["perm"].id, route_id=route.id)
