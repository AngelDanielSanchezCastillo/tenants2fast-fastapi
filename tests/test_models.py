...
async def test_tenant_slug_is_unique(session: AsyncSession):
    ...
    t1 = Tenant(name="Corp A", slug="unique-slug", database_name="tenant_corp_a", is_active=True)
    t2 = Tenant(name="Corp B", slug="unique-slug", database_name="tenant_corp_b", is_active=True)
